<div class="d-flex flex-row">
    <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton3">
            {{-- edit section --}}
            @can('edit_roles')
                <a href="{{ route('roles.edit', $role) }}" class="dropdown-item d-flex align-items-center">
                    <i data-feather="edit-2" class="icon-sm mr-2"></i>
                    <span class="">{{ __('change') }}</span>
                </a>
            @endcan
            @can('delete_roles')
                <button class="dropdown-item d-flex align-items-center" type="button" data-toggle="modal"
                    data-target="#deleteModal" data-role="{{ $role->id }}" data-name="{{ $role->name }}">
